...
const LoginPage = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('LoginPage: Attempting login with credentials:', credentials);
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        console.log('LoginPage: Login failed with error:', errorMessage);
        setError(errorMessage);
      }
    } catch (err) {
      console.error('LoginPage: Login error:', err);
      const errorMessage = err.response?.data?.error || err.response?.data?.detail || 'Invalid username or password';
