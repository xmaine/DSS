...
*   **Backend (Django):**
    *   Extend `Document` model with fields for `tags` (ManyToManyField or ArrayField), `description`, `document_type` (ForeignKey to a `DocumentType` model), `correspondent` (ForeignKey to a `Correspondent` model): **IN PROGRESS**
    ...
    *   Create API endpoints for managing `DocumentType` and `Correspondent` models (Admin/Senior Department Head scope): **IN PROGRESS**
*   **Frontend (React):**
...
*   **Backend (Django):**
    *   Define `Workflow` and `WorkflowStep` models: **TODO**
...
*   **Frontend (React):**
    *   UI for System Admin/Senior Department Head to define simple workflows: **TODO**
